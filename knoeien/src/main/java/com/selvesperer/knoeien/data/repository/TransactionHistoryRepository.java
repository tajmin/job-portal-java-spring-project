...
public interface TransactionHistoryRepository extends JpaRepository<TransactionHistory, String>, TransactionHistoryRepositoryCustom {
	...
	@Query(value="SELECT t.amount, t.created_date, t.created_by_name, j.title FROM transaction_history t join job j on t.job_id=j.id where t.to_user_id = :toUserId",nativeQuery=true)
